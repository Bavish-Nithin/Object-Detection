...
while cap.isOpened():
    ...
    blurred = cv2.GaussianBlur(color_frame, (21, 21), 0)
    ...
    diff_colour = cv2.absdiff(static_back, blurred)
    diff_flat = cv2.cvtColor(diff_colour, cv2.COLOR_BGR2GRAY)
    ...
    _, thresh_frame = cv2.threshold(diff_flat, 60, 255, cv2.THRESH_BINARY)
    ...
    cv2.imshow("2. Difference Map (absdiff)", diff_flat)
